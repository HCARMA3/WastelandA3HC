...
T8_varTLine01 = "Download cancelado!";				// download aborted
T8_varTLine02 = "Download está em processo por outra pessoa!";			// download already in progress by someone else
T8_varTLine03 = "Download iniciado!";					// download started
T8_varTLine04 = "Download finalizado! O dinheiro foi adicionado ao seu invetario!";				// download finished
T8_varTLine05 = "##  Hackear as contas Bancárias dos Jogadores  ##";				// line for the addaction
...
T8_fnc_ActionLaptop =
{
	...
	[ _laptop, _id] spawn 
	{
		...
		while { !T8_varDiagAbort && alive player && (player getVariable ["FAR_isUnconscious", 0] == 0)} do
		{
			...
			if ( _newFile > T8_varFileSize ) then 
			{
				...
				ctrlSetText [ 8001, "Download Finalizado!" ];	
				T8_varDiagAbort = true;
	...
	{	
		{    
			...
			_fivePercent = round(0.15*_bmoney);
	...
	default
	{
		{    
			...
			player setVariable ["cmoney", (player getVariable ["cmoney", 0]) + 1000000, true];
			[] spawn fn_savePlayerData;
			systemChat format["Você hackeou das contas bancárias o valor $1,000,000"];				
				};
			};
