	...
	//		diag_log format["[HSBlackmarket]: _randir: %1 _firstroad: %2",_randir,_firstroad];
		...
		[
			...
			[0,0,
				...
				["Land_LampAirport_F",[0.59082,-1.40234,1.90735e-006],0], 
...
if(hasInterface)then{
	...
	{
		_x addAction ["<img image='client\icons\rifle.paa'/> <t color='#ff0016'>Gun Store</t>",
		{
			[] call loadGunStore;
		},_x, -9, true, true, "", "player distance _target < 5"];
		
		_x addAction ["<img image='client\icons\store.paa'/> <t color='#16ff00'>General Store</t>",
		{
			[] call loadGeneralStore;
		},_x, -9, true, true, "", "player distance _target < 5"];
		
		_x addAction ["<img image='client\icons\atm.paa'/> <t color='#ffe900'>ATM</t>",
		{
